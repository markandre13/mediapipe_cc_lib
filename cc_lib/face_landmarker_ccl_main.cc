...
int face_landmarker_it() {
  ...
  //     cerr << "Failed to create FaceLandmarker: " <<
  //     landmarker.error().what() << endl; return 1;
  //   }
  if (!landmarker) {
  ...
  if (result->face_landmarks[0].landmarks.size() != 478) {
    cerr << "face landmarks: expected 478 landmarks for face, found "
         << result->face_landmarks[0].landmarks.size() << endl;
    return 1;
  }
  ...
  if (!cat.category_name.has_value() || *cat.category_name != "browDownLeft") {
    cerr << "face blendshapes: category[1].category_name to be \"browDownLeft\""
         << endl;
    return 1;
  }
  if (cat.display_name.has_value()) {
    cerr << "face blendshapes: category[1].display_name to not have a value"
         << endl;
    return 1;
  }
